...
foreach my $gid (@gene_ids){
	...
	my $start          = $genes{$gid}{'start'} - 1;
	my $end            = $genes{$gid}{'end'};
	my $gene_id        = $gid;
	my $transcript_ids = $genes{$gid}{'transcripts'};
	my $strand         = $genes{$gid}{'strand'};
	#
	my @segments = split /; /, $genes{$gid}{$TYPE};
	my ($exon_segments, $exon_length_str, $exon_length, $exon_number) = &search_overlap(\@segments);
	#
	print BED "$chr\t$start\t$end\t$gene_id\t$transcript_ids\t$strand\t$exon_length\t$exon_number\t$exon_segments\t$exon_length_str\n";
}
close BED;
...
sub search_overlap{
	...
	my @length       = ();
	my $total_length = 0;
	for(my $i=0;$i<scalar @mark_start;$i++){
		my $len = abs($mark_end[$i] - $mark_start[$i]) + 1;
		push @segments, "$mark_start[$i]-$mark_end[$i]";
		push @length, $len;
		$total_length += $len;
	}
	my $segments_str = join "; ", @segments;
	my $length_str   = join "; ", @length;
	my $seg_number   = scalar @length;
	return($segments_str, $length_str, $total_length, $seg_number)
}
